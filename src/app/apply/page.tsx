...
const processSteps = [
  ...
  { num: '03', label: 'Strategy Call', desc: '60-minute session to review findings and recommendations.' },
...
export default function ApplyPage() {
  ...
  const [form, setForm] = useState({
    ...
    budget: '', revenue: '', goals: '', timeline: '', extra: '',
  });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-end">
            ...
            <p className="text-base text-gray-500 leading-relaxed max-w-md">
              Complete the form below and we will perform a comprehensive audit of your existing ad accounts — completely free, no strings attached.
            </p>
          </div>
        </div>
      </section>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <select required value={form.budget} onChange={e => setForm({ ...form, budget: e.target.value })} className="w-full border border-gray-200 rounded-sm px-4 py-3 text-sm outline-none focus:border-black transition-colors text-gray-600 bg-white">
                    ...
                    <div>
                      ...
                      <select value={form.revenue} onChange={e => setForm({ ...form, revenue: e.target.value })} className="w-full border border-gray-200 rounded-sm px-4 py-3 text-sm outline-none focus:border-black transition-colors text-gray-600 bg-white">
                        <option value="">Select range</option>
                  ...
                  <div>
                    <label className="block text-xs tracking-widest uppercase text-gray-400 font-medium mb-3">Active Channels</label>
                    <div className="flex flex-wrap gap-2">
                      {channelOptions.map(c => (
                        <button
                          ...
                          className={`px-3 py-2 text-xs rounded-sm border transition-colors font-medium ${channels.includes(c) ? 'bg-black text-white border-black' : 'border-gray-200 text-gray-500 hover:border-gray-400'}`}
                        >
                          {c}
                        </button>
                      ))}
              ...
              <div>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <textarea required rows={3} value={form.goals} onChange={e => setForm({ ...form, goals: e.target.value })} placeholder="e.g. We want to scale from $50K to $200K/mo in ad spend while maintaining a 5x ROAS..." className="w-full border border-gray-200 rounded-sm px-4 py-3 text-sm outline-none focus:border-black transition-colors resize-none placeholder:text-gray-300" />
                  </div>
                  <div>
                    <label className="block text-xs tracking-widest uppercase text-gray-400 font-medium mb-2">Ideal Start Timeline</label>
                    <select value={form.timeline} onChange={e => setForm({ ...form, timeline: e.target.value })} className="w-full border border-gray-200 rounded-sm px-4 py-3 text-sm outline-none focus:border-black transition-colors text-gray-600 bg-white">
                  ...
                  <div>
                    ...
                    <textarea rows={3} value={form.extra} onChange={e => setForm({ ...form, extra: e.target.value })} placeholder="Context about your business, past agency experience, or specific challenges..." className="w-full border border-gray-200 rounded-sm px-4 py-3 text-sm outline-none focus:border-black transition-colors resize-none placeholder:text-gray-300" />
                  </div>
                </div>
              </div>

              <button type="submit" className="group w-full bg-black text-white text-xs tracking-widest uppercase font-semibold py-4 rounded-sm hover:bg-gray-900 transition-colors flex items-center justify-center gap-2">
              ...
              <p className="text-xs text-gray-400 text-center">
                By submitting, you agree to our privacy policy. We will respond within 1–2 business days.
              </p>
            </form>
          )}
        </div>
