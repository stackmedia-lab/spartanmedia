...
import { ArrowUpRight, TrendingUp, Target, Zap, BarChart3 } from 'lucide-react';
...
function StatBlock({ value, suffix, label, start }: { value: number; suffix: string; label: string; start: boolean }) {
  ...
  return (
    <div className="border-l border-gray-200 pl-8">
      <p className="text-4xl lg:text-5xl font-semibold tracking-tight text-black">{count}{suffix}</p>
      <p className="text-xs text-gray-500 tracking-widest uppercase mt-1">{label}</p>
    </div>
  );
}

const clients = ['Meta', 'Google', 'TikTok', 'Snapchat', 'LinkedIn', 'Pinterest', 'YouTube', 'Amazon'];
    ...
    <>
      ...
      <section className="min-h-screen flex flex-col justify-center pt-16 px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[85vh]">
            <div>
              <p className="text-xs tracking-widest uppercase text-gray-400 mb-6 font-medium">// Performance Media Buying</p>
              ...
              <div className="flex items-center gap-4">
                <Link href="/apply" className="group bg-black text-white text-xs tracking-widest uppercase font-semibold px-7 py-4 rounded-sm hover:bg-gray-900 transition-colors flex items-center gap-2">
                  Apply Now <ArrowUpRight size={13} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                </Link>
                <Link href="/case-studies" className="text-xs tracking-widest uppercase font-medium text-gray-400 hover:text-black transition-colors">
                  See Results →
                </Link>
              </div>
            </div>
              ...
              <div className="bg-[#0A0A0A] rounded-sm p-8 relative overflow-hidden">
                ...
                <div className="mt-8 pt-6 border-t border-white/10 relative z-10">
                  <div className="flex justify-between">
                    {[{ l: 'Avg ROAS', v: '7.4x' }, { l: 'Ad Spend', v: '$2.1M' }, { l: 'Revenue', v: '$15.5M' }].map(s => (
                      <div key={s.l}>
                        <p className="text-gray-500 text-xs uppercase tracking-wider">{s.l}</p>
                        <p className="text-white text-2xl font-semibold mt-0.5">{s.v}</p>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="border-y border-gray-100 py-4 overflow-hidden bg-[#0A0A0A]">
        <div className="marquee-track">
          {allClients.map((c, i) => (
            <span key={i} className="text-xs tracking-widest uppercase text-gray-400 mx-8 font-medium whitespace-nowrap">{c}</span>
          ))}
        </div>
      </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <p className="text-gray-500 text-base leading-relaxed max-w-md mb-8">
                We bridge the gap between ad spend and business outcomes through data-driven strategy, precise targeting, and relentless optimisation.
              </p>
              <Link href="/our-services" className="inline-flex items-center gap-2 text-xs tracking-widest uppercase font-semibold text-black hover:gap-3 transition-all">
      ...
      <section className="py-28 px-6 lg:px-8 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-end justify-between mb-12">
            ...
            <Link href="/case-studies" className="hidden sm:flex items-center text-xs tracking-widest uppercase font-medium text-gray-400 hover:text-black transition-colors">
              All Case Studies →
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { num: '04', label: 'Scale & Optimise', desc: 'Aggressive scaling of winning campaigns with daily optimisation.' },
            ].map(step => (
      ...
      <section className="py-28 px-6 lg:px-8 bg-[#0A0A0A]">
        <div className="max-w-7xl mx-auto">
          <p className="text-xs tracking-widest uppercase text-gray-500 mb-12 font-medium">Client Experiences</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { name: 'Marcus Chen', role: 'CMO, HealthBrands', quote: 'Spartan Media reduced our CAC by 52% within the first 60 days. Their systematic approach to media buying is unlike anything we have seen.' },
